...
function Sidebar({ active, setActive, onLogout, counts }: {
  ...
  return (
    <div style={{ width: '240px', background: '#0d1a0c', borderRight: '1px solid rgba(189,206,94,.1)', display: 'flex', flexDirection: 'column', flexShrink: 0, height: '100svh', position: 'sticky', top: 0 }}>
      {/* Logo */}
      <div style={{ padding: '24px 20px', borderBottom: '1px solid rgba(189,206,94,.08)' }}>
        ...
        {items.map(item => (
          ...
          >
            ...
            {item.count !== undefined && (
              <span style={{ marginLeft: 'auto', background: 'rgba(189,206,94,.15)', color: '#bdce5e', borderRadius: '10px', padding: '1px 8px', fontSize: '11px', fontWeight: 700 }}>
                {item.count}
              </span>
            )}
          </button>
      ...
      {/* Footer */}
      <div style={{ padding: '12px', borderTop: '1px solid rgba(189,206,94,.08)' }}>
        <a
          href="/" target="_blank"
          style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', borderRadius: '8px', color: 'rgba(245,242,235,.3)', textDecoration: 'none', fontFamily: "'DM Sans',sans-serif", fontSize: '12px', marginBottom: '4px' }}
        >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: '20px', marginBottom: '40px' }}>
        {cards.map((c, i) => (
          ...
          >
            ...
            <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '12px', color: 'rgba(245,242,235,.4)', marginTop: '4px', letterSpacing: '1px' }}>{c.label}</div>
          </motion.div>
        ))}
      </div>
...
function EditModal({ title, children, onClose, onSave, saving }: any) {
  return (
    ...
    >
      <motion.div
        initial={{ scale: .93, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: .95, opacity: 0 }}
...
function ProjectsManager() {
  ...
  const blank = { title: '', tag: '', icon: 'Rocket', description: '', techs: '', link: '', link_label: '', featured: false, status: 'En développement' }
  ...
  return (
    <div style={{ padding: '40px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        ...
        <motion.button onClick={() => setEditing(blank)} whileHover={{ scale: 1.04 }} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#bdce5e', color: '#254f24', border: 'none', borderRadius: '8px', padding: '12px 20px', fontFamily: "'DM Sans',sans-serif", fontSize: '12px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {projects.map(p => (
            <motion.div key={p.id} whileHover={{ x: 4 }} style={{ display: 'flex', alignItems: 'center', gap: '16px', background: '#0d1a0c', border: '1px solid rgba(189,206,94,.08)', borderRadius: '10px', padding: '16px 20px' }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '14px', fontWeight: 700, color: '#f5f2eb', marginBottom: '3px' }}>{p.title}</div>
                <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '12px', color: 'rgba(245,242,235,.4)' }}>{p.tag} · {p.status}</div>
              </div>
              {p.featured && <span style={{ background: 'rgba(189,206,94,.15)', color: '#bdce5e', padding: '3px 10px', borderRadius: '20px', fontSize: '9px', letterSpacing: '2px', textTransform: 'uppercase', fontFamily: "'DM Sans',sans-serif", fontWeight: 700 }}>Featured</span>}
        ...
        {editing && (
          <EditModal title={editing.id ? 'Modifier le projet' : 'Nouveau projet'} onClose={() => setEditing(null)} onSave={handleSave} saving={saving}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <input type="checkbox" id="featured" checked={editing.featured} onChange={e => setEditing({ ...editing, featured: e.target.checked })} style={{ width: '16px', height: '16px', accentColor: '#bdce5e' }}/>
      ...
      <AnimatePresence>
        {editing && (
          <EditModal title={editing.id ? "Modifier l'article" : 'Nouvel article'} onClose={() => setEditing(null)} onSave={handleSave} saving={saving}>
            ...
            <FormField label="URL image"       value={editing.image_url} onChange={(v: string) => setEditing({ ...editing, image_url: v })}/>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <input type="checkbox" id="pub" checked={editing.published} onChange={e => setEditing({ ...editing, published: e.target.checked })} style={{ width: '16px', height: '16px', accentColor: '#bdce5e' }}/>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {videos.map(v => (
            <motion.div key={v.id} whileHover={{ x: 4 }} style={{ display: 'flex', alignItems: 'center', gap: '16px', background: '#0d1a0c', border: '1px solid rgba(189,206,94,.08)', borderRadius: '10px', padding: '16px 20px' }}>
              {v.thumbnail_url && <img src={v.thumbnail_url} alt={v.title} style={{ width: '80px', height: '45px', objectFit: 'cover', borderRadius: '6px', flexShrink: 0 }}/>}
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: '14px', fontWeight: 700, color: '#f5f2eb', marginBottom: '3px' }}>{v.title}</div>
      ...
      <AnimatePresence>
        {editing && (
          <EditModal title={editing.id ? 'Modifier la vidéo' : 'Nouvelle vidéo'} onClose={() => setEditing(null)} onSave={handleSave} saving={saving}>
            ...
            <FormField label="Description"    value={editing.description}   onChange={(v: string) => setEditing({ ...editing, description: v })} multiline/>
            <FormField label="URL miniature"  value={editing.thumbnail_url} onChange={(v: string) => setEditing({ ...editing, thumbnail_url: v })}/>
...
export default function AdminPage() {
  ...
  useEffect(() => {
    getAdminSession().then(session => setAuthenticated(!!session))
  }, [])
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100svh', background: '#0a0f0a', color: '#f5f2eb' }}>
